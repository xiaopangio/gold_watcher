...
func (app *Config) getPriceText() (*canvas.Text, *canvas.Text, *canvas.Text) {
	...
	if err != nil {
		...
		openTxt := fmt.Sprintf("Open: $%.2f %s", gold.PreviousClose, currency)
		currentTxt := fmt.Sprintf("Current: $%.2f %s", gold.Price, currency)
		changeTxt := fmt.Sprintf("Change: $%.2f %s", gold.Change, currency)
